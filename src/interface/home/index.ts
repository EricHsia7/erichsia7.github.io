...
function generateElementOfRepository(repo: Repository): HTMLElement {
  ...
  element.innerHTML = `<div class="css_home_repository_ogimage">${createOGImage(repo.ogImage)}</div><div class="css_home_repository_content"><div class="css_home_repository_name">${repo.full_name}</div><div class="css_home_repository_desc">${repo.description}</div><div class="css_home_repository_languages_and_status">${languages}<div class="css_home_repository_active_status" status="${status}">${text}</div></div><div class="css_home_repository_tags" len="${repo.tags.length}">${tags}</div>${recentEvents}</div><div class="css_home_repository_github"><a href="https://github.com/${repo.full_name}"><div class="css_home_repository_github_link">View on GitHub</div></a></div>`;
  return element;
}
